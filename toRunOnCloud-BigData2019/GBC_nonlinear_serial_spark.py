...
spark = SparkSession \
    .builder \
    .appName("GBC_nonlinear_serial_spark") \
    .getOrCreate()
...
def boosting(x_list_name, y_name, maxlag, data=data_input):
    ...
    for pivot_x in range(0, len(x_list_name)):
        print("=========this is regression round {}=========in def boosting()==".format(pivot_x + 1))
        timestamp_inside_for_loop_in_boosting = datetime.now()
        print("starting at {}".format(timestamp_inside_for_loop_in_boosting))
        ...
        if pivot_x == 0:
            ...
            # print("n is ")
            # print(n)
            print("check point 1 inside def boosting ...")
            boositing_ts = datetime.now()
            print("start join...")
            print("create rid for res_col")
            ...
            data = data.drop('rid')
            print("finished join...")
            print(datetime.now() - boositing_ts)
            res_list = regression(x_list_name[pivot_x], y_name, maxlag, data)

        print('regression time used: ')
        print(datetime.now() - timestamp_inside_for_loop_in_boosting)
...
with open("output_nonlinear_serial.csv", "w", newline='') as f:
    for row in result:
        f.write("%s\n" % ','.join(str(col) for col in row))

with open('output_nonlinear_serial.csv') as input, open('output_nonlinear_serial_{}.csv'.format(data_file_name), 'w', newline='') as output:
